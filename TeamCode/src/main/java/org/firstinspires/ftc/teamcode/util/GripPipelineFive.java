...
* @author GRIP
*/
public class GripPipelineFive extends OpenCvPipeline{
...
/*
	double[] hsvThresholdHue = {17, 72.0};
	double[] hsvThresholdSaturation = {174, 255.0};
	double[] hsvThresholdValue = {89, 255};
*/

	public double[][] contourPoints;

	public GripPipelineFive(Telemetry telemetry) {
		resizeImageOutput = new Mat();
		hsvThresholdOutput = new Mat();
		desaturateOutput = new Mat();
	...
	public Mat processFrame(Mat source0) {
		...
		// Step HSV_Threshold0:
		double[] hsvThresholdHue = {minH, maxH};
		double[] hsvThresholdSaturation = {minS, maxS};
		double[] hsvThresholdValue = {minV, maxV};
		...
		Mat Output = resizeImageOutput;

		Imgproc.drawContours(Output, filterContoursOutput, -1, new Scalar(0, 255, 0));

		telemetry.addData("Contours", filterContoursOutput.size());
		contourPoints = new double[filterContoursOutput.size()][2];
		
		int i = 0;

		for( MatOfPoint mop: filterContoursOutput){
			telemetry.addData("num", mop.toList().size());
    			for( Point p: mop.toList() ){
				contourPoints[i][0] += p.x;
				contourPoints[i][1] += p.y;
				
				//telemetry.addData("x", contourPoints[i][0]); // another coordinate
				//telemetry.addData("y", contourPoints[i][1]);

        			//telemetry.addData("x", p.x); // another coordinate
				//telemetry.addData("y", p.y);
    			}

			contourPoints[i][0] /= mop.toList().size();
			contourPoints[i][1] /= mop.toList().size();

			telemetry.addData("x", contourPoints[i][0]); // another coordinate
			telemetry.addData("y", contourPoints[i][1]);

			Imgproc.rectangle( Output,
                				new Point( contourPoints[i][0], contourPoints[i][1]),
                				new Point( contourPoints[i][0]+1, contourPoints[i][1]+1),
                				new Scalar( 0, 255, 255 ), -1, 8, 0 );
